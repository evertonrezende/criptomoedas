...
def bt_rsi():
    ...
    if not t_grafico.get() and moeda.get() :
        tkinter.messagebox.showinfo('Erro', 'voce preencheu somente a moeda!!')


    if not moeda.get().isalpha(): #valida  da moedase foi digitado apendas letras
        tkinter.messagebox.showinfo('Erro', 'Moeda com campos inválidos!!') # exibe a mensagem de erro
        lb2['text'] = ' ' # limpa o label     
    else:
