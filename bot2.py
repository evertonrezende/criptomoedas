...
def rsi_tradingview(param):
    ...
    for cont in range(0, 7):
        if co == 0:
            ...
            rsi_1d = float(indicador[0])

        elif co == 2:
            candle = '240' # candle de 4 horas
            f = "RSI|{}"
            payload = {
                "symbols": {
                    "tickers": ["BINANCE:{}".format(market)],
            ...
            indicador = resp["data"][0]["d"]
            #print(f'RSI: {indicador[0]}')  # RSI 14 periodos
            rsi_4h = float(indicador[0])

        elif co == 3:
            candle = '60' #candle de 60 min
            f = "RSI|{}"
            payload = {
                "symbols": {
                    "tickers": ["BINANCE:{}".format(market)],
            ...
            indicador = resp["data"][0]["d"]
            #print(f'RSI: {indicador[0]}')  # RSI 14 periodos
            rsi_1h = float(indicador[0])

        elif co == 4:
            candle = '15' #candle de 15 min
            f = "RSI|{}"
            payload = {
                "symbols": {
                    "tickers": ["BINANCE:{}".format(market)],
            ...
            indicador = resp["data"][0]["d"]
            #print(f'RSI: {indicador[0]}')  # RSI 14 periodos
            rsi_15m = float(indicador[0])

        elif co == 5:
            candle = '5' # cancle de 5 min
            f = "RSI|{}"
            payload = {
                "symbols": {
                    "tickers": ["BINANCE:{}".format(market)],
            ...
            indicador = resp["data"][0]["d"]
            #print(f'RSI: {indicador[0]}')  # RSI 14 periodos
            rsi_5m = float(indicador[0])

        elif co == 6:
            candle = '1' #candle de 1 min
            f = "RSI|{}"
            payload = {
                "symbols": {
                    "tickers": ["BINANCE:{}".format(market)],
    ...
    if (param == 'BTC'):
        ...
        par = '/BTC'
    mandarMensagem(idUsuario, '<b>RSI ' + param + par + '- BINANCE</b>' + '\n\n' +
                                     '<b>1W : </b> {:.2f}'.format(rsi_1w) + '\n' +
                                     '<b>1d : </b> {:.2f}'.format(rsi_1d) + '\n' +
                                     '<b>4h : </b> {:.2f}'.format(rsi_4h) + '\n' +
                                     '<b>1h : </b> {:.2f}'.format(rsi_1h) + '\n' +
                                     '<b>15m : </b> {:.2f}'.format(rsi_15m) + '\n' +
                                     '<b>5m : </b> {:.2f}'.format(rsi_5m) + '\n' +
                                     '<b>1m : </b> {:.2f}'.format(rsi_1min)
                           )
...
def ma_tradingview(param):
    ...
    mandarMensagem(idUsuario,
                   '<b>SMA ' + '(200 períodos) ' + param + par + '- BINANCE</b>' + '\n' +
                   '1d : ' + sma_1[0] + '\n' +
                   '4h : ' + sma_1[1] + '\n' +
                   '1h : ' + sma_1[2] + '\n' +
                   '<b>SMA ' + '(100 períodos) ' + param + par + '- BINANCE</b>' + '\n' +
                   '1d : ' + sma_2[0] + '\n' +
                   '4h : ' + sma_2[1] + '\n' +
                   '1h : ' + sma_2[2] + '\n' +
                   '<b>SMA ' + '(20 períodos) ' + param + par + '- BINANCE</b>' + '\n' +
                   '1d : ' + sma_3[0] + '\n' +
                   '4h : ' + sma_3[1] + '\n' +
                   '1h : ' + sma_3[2]+ '\n')
...
def rsi_t(candle):
    ...
    if candle == '1s' or candle == '1S':
        candleBinance = '1W'
        f = 'RSI|{}'
        pr = '|1W'


    elif candle == '1d' or candle == '1D':
        candleBinance = '1d'
        f = 'RSI'
        pr = ''
    elif candle == '4h' or candle == '4H':
        candleBinance = '240'
        f = 'RSI|{}'
        pr = '|240'

    elif candle == '1h' or candle == '1H':
        candleBinance = '60'
        f = 'RSI|{}'
        pr = '|60'
    elif candle == '15m' or candle == '15M':
        candleBinance = '15'
        f = 'RSI|{}'
        pr = '|15'

    elif candle == '5m' or candle == '5M':
        candleBinance = '5'
        f = 'RSI|{}'
        pr = '|5'
    elif candle == '1m' or candle == '1M':
        candleBinance = '1'
        f = 'RSI|{}'
        pr = '|1'
    payload = {
        ...
        "columns":
            ...
            ["name", f.format(candleBinance), "description", "name", "subtype", "RSI", "RSI[1]" + pr],
    ...
    resp = requests.post(url, headers=headers, data=json.dumps(payload2)).json()
    print(resp)
    n2 = 0
    n2_rsi = []
    v2_rsi = []
    for cont in range(0, 10):
        moeda_name2 = resp["data"][n2]["d"][0]
        moeda_price2 = resp["data"][n2]["d"][1]
        n2_rsi.append(moeda_name2)
        v2_rsi.append(float(moeda_price2))

        n2 = n2 + 1


    mandarMensagem(idUsuario, '<b>RSI ' + candle + ' - BINANCE - menores</b>' + '\n\n' +
                   '<b> ' + n_rsi[0] + ' : ' + '</b> {:.2f}'.format(v_rsi[0]) + '\n' +
                   '<b> ' + n_rsi[1] + ' : ' + '</b> {:.2f}'.format(v_rsi[1]) + '\n' +
                   '<b> ' + n_rsi[2] + ' : ' + '</b> {:.2f}'.format(v_rsi[2]) + '\n' +
                   '<b> ' + n_rsi[3] + ' : ' + '</b> {:.2f}'.format(v_rsi[3]) + '\n' +
                   '<b> ' + n_rsi[4] + ' : ' + '</b> {:.2f}'.format(v_rsi[4]) + '\n' +
                   '<b> ' + n_rsi[5] + ' : ' + '</b> {:.2f}'.format(v_rsi[5]) + '\n' +
                   '<b> ' + n_rsi[6] + ' : ' + '</b> {:.2f}'.format(v_rsi[6]) + '\n' +
                   '<b> ' + n_rsi[7] + ' : ' + '</b> {:.2f}'.format(v_rsi[7]) + '\n' +
                   '<b> ' + n_rsi[8] + ' : ' + '</b> {:.2f}'.format(v_rsi[8]) + '\n' +
                   '<b> ' + n_rsi[9] + ' : ' + '</b> {:.2f}'.format(v_rsi[9]) + '\n''\n' +
                   '<b>RSI ' + candle + ' - BINANCE - maiores</b>' + '\n\n' +
                   '<b> ' + n2_rsi[0] + ' : ' + '</b> {:.2f}'.format(v2_rsi[0]) + '\n' +
                   '<b> ' + n2_rsi[1] + ' : ' + '</b> {:.2f}'.format(v2_rsi[1]) + '\n' +
                   '<b> ' + n2_rsi[2] + ' : ' + '</b> {:.2f}'.format(v2_rsi[2]) + '\n' +
                   '<b> ' + n2_rsi[3] + ' : ' + '</b> {:.2f}'.format(v2_rsi[3]) + '\n' +
                   '<b> ' + n2_rsi[4] + ' : ' + '</b> {:.2f}'.format(v2_rsi[4]) + '\n' +
                   '<b> ' + n2_rsi[5] + ' : ' + '</b> {:.2f}'.format(v2_rsi[5]) + '\n' +
                   '<b> ' + n2_rsi[6] + ' : ' + '</b> {:.2f}'.format(v2_rsi[6]) + '\n' +
                   '<b> ' + n2_rsi[7] + ' : ' + '</b> {:.2f}'.format(v2_rsi[7]) + '\n' +
                   '<b> ' + n2_rsi[8] + ' : ' + '</b> {:.2f}'.format(v2_rsi[8]) + '\n' +
                   '<b> ' + n2_rsi[9] + ' : ' + '</b> {:.2f}'.format(v2_rsi[9])
                   )
...
def pegaUltimaMensagem(msg):
    ...
    elif msg['text'].find('ema') != -1 or msg['text'].find('EMA') != -1:
        ma_moeda = msg['text']
        ma_moeda = ma_moeda[3:10]
        ma_moeda = ma_moeda.upper()
        print(ma_moeda)
    ...
    elif msg['text'].find('rsi 1s') != -1 or msg['text'].find('rsi 1S') != -1 or msg['text'].find('rsi 1d') != -1 or msg['text'].find('RSI 1D') != -1 or msg['text'].find('rsi 4h') != -1 or msg['text'].find('rsi 4H') != -1 or msg['text'].find('rsi 1h') != -1 or msg['text'].find('rsi 1H') != -1 or msg['text'].find('rsi 15m') != -1 or msg['text'].find('rsi 15M') != -1 or msg['text'].find('rsi 5m') != -1 or msg['text'].find('rsi 5M') != -1 or msg['text'].find('rsi 1m') != -1 or msg['text'].find('rsi 1M') != -1 :
        modif = msg['text']
        modif = modif[4:8]
        #modif = modif.upper()
        print(modif)
